...
function Spinner() {
  return (
    <span className="inline-block w-4 h-4 border-2 border-white/20 border-t-[#04F5FF] rounded-full animate-spin" />
  )
}
...
function MatchHeader({ fixture }: { fixture: Fixture }) {
  ...
  return (
    <div className="mb-10">
      <Link to="/" className="text-[11px] text-[#6B3F7E] hover:text-[#9ca3af] transition-colors">
        ← Back to fixtures
      </Link>
      <div className="mt-4 flex items-center gap-2 text-[10px] uppercase tracking-widest text-[#6B3F7E] font-medium">
        ...
        {fixture.status === 'completed' && (
          <>
            <span>·</span>
            <span className="text-[#9D79BC]">Full time</span>
          </>
        )}
      </div>
      <h1 className="mt-3 flex items-center gap-3 text-xl font-semibold tracking-tight">
        ...
        <span className="text-[#6B3F7E] font-normal text-lg">vs</span>
...
function ScoreStepper({ team, value, onChange }: StepperProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <span className="text-[11px] text-[#9D79BC] font-medium text-center max-w-[80px] truncate">
        {team}
      </span>
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-center gap-6">
        ...
        <span className="text-3xl text-[#6B3F7E] mb-2 font-light select-none">—</span>
    ...
    <div className="flex flex-col items-center gap-3 w-full">
      ...
      <div className="flex items-center justify-between w-full px-1">
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-[10px] text-[#9D79BC] text-center leading-tight max-w-[60px] truncate">
            {homeTeam}
          </span>
        </div>
...
function WinBar({ p_home_win, p_draw, p_away_win, homeTeam, awayTeam }: {
  ...
  return (
    <div className="mt-4">
      ...
      <div className="flex justify-between mt-1.5 text-[10px] text-[#6B3F7E]">
...
function FullTimeScore({ fixture }: { fixture: Fixture }) {
  return (
    <div className="mt-10 mb-2 px-1">
      <p className="text-[10px] uppercase tracking-widest text-[#6B3F7E] mb-8 font-medium text-center">
        Full time
      </p>
...
function SeasonTotals({ totals }: { totals: LeaderboardEntry }) {
  ...
  return (
    <div className="mt-5 p-5 bg-white/[0.05] border border-white/[0.08] rounded-xl">
      <p className="text-[10px] uppercase tracking-widest text-[#6B3F7E] mb-4 font-medium">
        Season — {totals.matches_played} {totals.matches_played === 1 ? 'match' : 'matches'}
      </p>
      <div className="flex items-center justify-between gap-4">
        <div className="flex-1 text-center">
          <p className="text-[10px] uppercase tracking-widest text-[#6B3F7E] mb-1.5">You</p>
          <p className={`text-3xl font-bold tabular-nums ${userAhead ? 'text-[#00FF85]' : 'text-white'}`}>
            {totals.total_user_score}
          </p>
          {userAhead && <p className="text-[10px] text-[#00FF85] mt-1">leading</p>}
        </div>
        <div className="text-[#6B3F7E] text-sm font-light select-none">vs</div>
        <div className="flex-1 text-center">
          <p className="text-[10px] uppercase tracking-widest text-[#6B3F7E] mb-1.5">Model</p>
          <p className={`text-3xl font-bold tabular-nums ${modelAhead ? 'text-[#00FF85]' : 'text-[#9D79BC]'}`}>
            {totals.total_model_score}
          </p>
          {modelAhead && <p className="text-[10px] text-[#00FF85] mt-1">leading</p>}
        </div>
      </div>
    </div>
  )
      ...
      <div className="grid grid-cols-2 gap-4" style={{ perspective: '1200px' }}>
        ...
        <div className="relative" style={{ minHeight: '160px' }}>
          <div className={`flip-card-inner h-full ${flipped ? 'flipped' : ''}`}>
            ...
            <div className="flip-card-face bg-white/[0.05] border border-white/[0.08] p-5 flex flex-col items-center justify-center gap-3">
              <div className="w-5 h-5 border-2 border-white/10 border-t-[#04F5FF] rounded-full animate-spin" />
            ...
            <div className="flip-card-face flip-card-back bg-white/[0.05] border border-[#04F5FF]/25 p-5">
              <p className="text-[10px] uppercase tracking-widest text-[#04F5FF] mb-5 font-medium">
                The model
              </p>
              {modelPred ? (
                <>
                ...
                <p className="text-xs text-[#9D79BC]">Unavailable</p>
              )}
            </div>
          </div>
        </div>
...
export default function PredictionPage() {
  ...
  if (phase.type === 'loading') {
    return <p className="text-[#6B3F7E] text-sm">Loading…</p>
  }
  ...
  if (phase.type === 'submitting') {
    return (
      <div>
        ...
        <div className="flex items-center justify-center gap-3 py-16 text-[#9D79BC] text-sm">
          <Spinner />
