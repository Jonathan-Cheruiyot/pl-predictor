...
export default function Nav() {
  ...
  const isActive = (path: string) =>
    path === '/'
      ? pathname === '/' || pathname.startsWith('/fixtures/')
      : pathname === path || pathname.startsWith(path + '/')

  const linkClass = (path: string) =>
    `text-sm transition-colors ${isActive(path) ? 'text-[#04F5FF]' : 'text-[#9D79BC] hover:text-white'}`
    ...
    <header className="border-b border-white/[0.08] px-4">
      <div className="mx-auto max-w-2xl flex items-center justify-between h-14">
        ...
        <nav className="flex items-center gap-6">
          ...
          <Link to="/past" className={linkClass('/past')}>Past</Link>
